		...
		case SC_PutString :
			{
				...
				int size = consoledriver->length(from);
				...
				if(size <= MAX_STRING_SIZE){
					...
					int i =1;
					while(size > MAX_STRING_SIZE*i){
						consoledriver->copyStringFromMachine (from +(MAX_STRING_SIZE*(i-1)), buffer, MAX_STRING_SIZE+1);
						consoledriver->PutString(buffer);
						i++;
					}
					consoledriver->copyStringFromMachine (from +(MAX_STRING_SIZE*(i-1)), buffer, size - MAX_STRING_SIZE*i +1);
					consoledriver->PutString(buffer);
				}
